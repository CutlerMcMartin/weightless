    ...
    <?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
        <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
            <header class="header">
                ...
                <a href="<?php echo esc_url( get_permalink( $post->post_parent ) ); ?>" title="<?php printf( esc_html__( 'Return to %s', 'weightless' ), esc_html( get_the_title( $post->post_parent ), 1 ) ); ?>" rev="attachment"><?php printf( esc_html__( '%s Return to ', 'weightless' ), '<span class="meta-nav">&larr;</span>' ); ?><?php echo get_the_title( $post->post_parent ); ?></a>
